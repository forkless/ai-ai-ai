...
function Show-Status {
    ...
    # GPU utilization — discover available counters first, probe only what exists
    $gpuCounters = try { Get-Counter -ListSet "GPU*" -ErrorAction Stop | Select-Object -ExpandProperty Paths } catch { @() }
    $gpuUtil = $null
    if ($gpuCounters -match 'Utilization Percentage') {
    ...
    # VRAM used — probe only if the counter exists
    $gpuVramUsed = $null
